...
export default class IncidentPage extends Vue {
  ...
  public getIncidentStatus (): string {
    ...
    return this.incident.closedAt ? `${this.$t('Closed on')} ${(inLast24Hours(this.incident.closedAt) ? formatDateTimeWithoutYear : toHumanDateStr)(this.incident.closedAt, this.stream?.timezone ?? 'UTC')} by ${username}` : 'Mark as closed'
  }
